...
def analyze(
    ...
    return (
        run_command(command, cwd=build.path, check=False, timeout=90),
        log_path,
        build,
    )
...
def get_already_analyzed(success_builds, previous_state_file_path):
    ...
    if previous_state_file_path:
        return [
            a
            for s in load(previous_state_file_path)
            if s.step == "schmetterling.analyze.sonarqube"
            for a in s.analyzes
...
def get_not_analyzed_builds(success_builds, already_analysed):
    return [
        b for b in success_builds if b.path not in [p.path for p in already_analysed]
    ]
...
def execute(
    ...
    already_analyzed = get_already_analyzed(success_builds, previous_state_file_path)
    not_analyzed_builds = get_not_analyzed_builds(success_builds, already_analyzed)
