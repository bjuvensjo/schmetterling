...
def create_state(repos, paths, statuses, branches, setup_branch):
    return SetupState(__name__, [
        Repo(
            ...
            path,
            [
                Branch(b['name'], b['name'] == repo['defaultBranch'].split('/')
                       [-1], b['objectId'])
    ...
    ]) if repos else SetupState(__name__, None)
...
def filter_repos(repos, branches, projects, setup_branch):
    return [
        repo for repo in repos if is_included(
            repo['name'],
...
def get_branches(repos):
    return dict(
        vang_get_branches(
            repos=[f'{r["project_spec"]}/{r["name"]}' for r in repos]))
...
def get_repo_paths(repos, work_dir):
    return [
        f'{work_dir}/{repo["project_spec"]}/{repo["name"]}' for repo in repos
    ]
...
def sync_repo(work_dir, branches, setup_branch, repo_and_repo_path):
    ...
    return update_repo(
        ...
        repo,
        repo_path,
    ) if exists(repo_path) else clone_repo(work_dir, setup_branch, repo,
                                           repo_path)
...
def sync_repos(repos, repo_paths, work_dir, branches, setup_branch):
    return pmap(
        partial(sync_repo, work_dir, branches, setup_branch),
...
def get_previous_state_repo_heads(previous_state):
    return {
        r.path: b.head
        for s in previous_state if s.step == 'schmetterling.setup.tfs'
        for r in s.repos for b in r.branches if b.name == r.setup_branch
    }
...
def get_repo_statuses(repo_paths, previous_state_file_path):
    if previous_state_file_path:
        ...
        return [
            get_commit(p) != previous_state_heads.get(p, None)
            for p in repo_paths
        ]
...
def execute(state, url, username, password, setup_dir, setup_branch, projects):
    ...
    setup_state = create_state(repos, repo_paths, repo_statuses, branches,
                               setup_branch)
    return setup_state
