...
def get_branches(repos):
    return dict(
        vang_get_branches(
            [(repo['project']['key'], repo['slug']) for repo in repos]))
...
def get_repo_paths(repos, work_dir):
    return [
        f'{work_dir}/{repo["project"]["key"]}/{repo["slug"]}' for repo in repos
    ]
...
def sync_repo(work_dir, branches, setup_branch, repo_and_repo_path):
    ...
    return update_repo(
        ...
        repo,
        repo_path,
    ) if exists(repo_path) else clone_repo(
        work_dir,
        setup_branch,
        repo,
    )
...
def execute(state, url, username, password, setup_dir, setup_branch, projects):
    ...
    repo_statuses = sync_repos(repos, repo_paths, work_dir, branches,
                               setup_branch)
